...
function findMinMax([firstElement, ...restElements]) {
  ...
  for (const element of restElements) {
    if (element < min) {
      ...
      min = element; // Якщо так, оновлюємо мінімальне значення
    }
    if (element > max) {
      // Перевіряємо, чи поточний елемент більший за максимальне значення
      max = element; // Якщо так, оновлюємо максимальне значення
    }
  }
...
function multiplyByIndex(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    // Проходимо через кожен елемент вхідного масиву
    result[result.length] = arr[i] * i; // Помножуємо число на його індекс і додаємо до масиву результату
  }
  return result;
  // Повертаємо масив результату
}
...
function replaceNumbers(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] > 10) {
      result[result.length] = "Greater than 10";
    } else {
      result[result.length] = "Less than or equal to 10";
    }
  } // Проходимо через кожен елемент вхідного масиву
